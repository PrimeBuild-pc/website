...
const FAQSection = () => {
  ...
  return (
    <section id="faq" className="section-shell relative overflow-hidden bg-black">
      <div className="site-container">
        <AnimatedElement>
          <SectionHeader
            eyebrow="FAQ"
            title="Domande frequenti,"
            highlight="risposte chiare"
        ...
        <div className="mx-auto max-w-4xl space-y-3">
          {faqs.map((faq, index) => (
            <AnimatedElement key={index} delay={0.05 * index}>
              <div className={`overflow-hidden rounded-2xl border transition-colors ${openIndex === index ? "border-primary/30 bg-primary/[0.045]" : "border-white/10 bg-white/[0.035]"}`}>
                <button
                  onClick={() => toggleFAQ(index)}
                  className="flex w-full items-center justify-between gap-4 px-5 py-5 text-left transition-colors hover:bg-white/[0.035] sm:px-6"
                  aria-expanded={openIndex === index}
                  aria-controls={`faq-answer-${index}`}
                >
                  <span className="font-semibold text-white">{faq.question}</span>
                  <span className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-full border border-white/10 bg-white/5 text-primary transition-transform duration-300 ${openIndex === index ? 'rotate-180' : ''}`}>
                    <ChevronDown className="h-4 w-4" aria-hidden="true" />
                  </span>
                </button>
                <div
                  id={`faq-answer-${index}`}
                  className={`overflow-hidden transition-all duration-300 ${openIndex === index ? 'max-h-[1000px] pb-6' : 'max-h-0'}`}
                >
                  <p className="px-5 leading-7 text-neutral-400 sm:px-6">{faq.answer}</p>
                </div>
              </div>
            </AnimatedElement>
          ))}
        ...
        <AnimatedElement className="text-center mt-12" delay={0.4}>
          <p className="text-neutral-400 mb-5">Hai altre domande?</p>
